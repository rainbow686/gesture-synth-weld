...
- **Flow**: Record button → mode/ratio chooser (SVG previews) → 3s countdown (cyan "Get ready" 3-2-1) → 120s recording (button counts down remaining 120→0; last 3s: magenta "Wrap up" 3-2-1 overlay, DOM-only — never in the video; `RECORD_SECONDS` constant, 15 → 30 (08-09) → 60 (08-17) → 120 (09-01, timeout plateaued ~27%)) → result panel
...
**v2.3** - Recording up to 120s (timeout-driven, announced via whatsNew)
- Prior v2.2: My works local gallery + 60s recording + PR#1 (keymap/QWERTZ/slider switch/natural diatonic)
