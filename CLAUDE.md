...
- **Visual atmosphere (2026-08-04)**: stage lighting — Vignette (≤28% edge darkening) or Scanlines (≤4% texture), **on by default at 50%** (= the pre-slider fixed look; saved in localStorage gsw-vignette/gsw-scanlines, user choice wins). Live view: pure CSS overlay (display layer only, gesture pipeline untouched). Recording: drawn inside the window in drawRecFrame (design bands stay clean). WYSIWYG between live and recording
