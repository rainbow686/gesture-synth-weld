...
- **Toolbar camera↔keyboard switch** (desktop only, mobile-collapse — keyboard mode is desktop-only so the switch has nothing to switch to on phones): the PERMANENT prominent mode switch (user decision 2026-08-09 — the NEW card expires after its announce window, this button doesn't; keyboard-mode players must always find the way back to the camera). **Always a labeled capsule in both states — one control, one shape** (user decision 2026-08-09: capsule in one mode + circle in the other read as two controls; text labels avoid icon confusion with stop-camera): keyboard mode = "📷 Camera", camera mode = "⌨ Keyboard"; fixed min-width 96px so the centered toolbar never shifts. **Switching while playing goes through stopCamera() FIRST, then the target starter** (bug 2026-08-09: keyboard→camera used to start the camera directly over the resident keyboard session — the old stream + MediaPipe session stayed live and gestures came back dead; camera→keyboard used to leave the camera stream running, LED on. Both directions now mirror the provably-working fresh path: isRunning false → true). It **pulses while the What's-new card is visible** (the card teaches it, the glow points at it). The old **stop-camera button is hidden in keyboard mode** (visibility:hidden — keeps the slot so the toolbar doesn't shift; no camera to stop — misleading; the Camera capsule takes its role). In camera mode stop stays — the only way out of the playing scene. Help keyboard section + first-run KbGuide hint line both point at the switch
...
- **Mechanism** (`src/whatsNew.ts`): newest entry at top of WHATS_NEW (version/releasedAt/title/body) — **ONE entry is live at a time**: adding a new one retires the previous (new version key = the playing-scene card reappears even for players who dismissed the old one). **Two touchpoints, two mechanisms** (user decision 2026-08-09 — each maps to a different moment in the journey):
  ...
  - Help modal shows "New in this version" permanently (changelog role) — reading it dismisses nothing. **Dismissed state mirrored in React** (`whatsNewDismissedState` in App — localStorage alone can't trigger a re-render). Card copy is one line per entry (v2.2: "My works — saved in this browser")
