...
- **First-run keyboard guide** (`src/components/KbGuide.tsx`): REAL physical QWERTY layout — position recognition + live press-to-learn highlighting (pressed key glows, shows degree/function; cyan=harmony, pink=expression, red=stop, unmapped keys dim). **Auto-demo animation** on open: cycles every mapped key (1.4s each, caption "按 3 → III 级"), pauses the moment the player presses a mapped key themselves. Auto-shows once (localStorage gsw-keyboard-guide-seen); first-run auto-pop hides after 8s, **Help-modal replay does NOT auto-hide** (the player asked to see it — closes on click/Esc only). (Abstract mini-keypad retired 2026-08-09 — user feedback: it flashed away too fast and didn't map to real key positions)
